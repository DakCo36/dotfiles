...
RSpec.describe Component::BatComponent do
  ...
  describe "#setup_completions" do
    ...
    it "creates directories and copies completion files" do
      ...
      expect(bat).to have_received(:runCmd).with("cp", "/tmp/test/bat-assets/autocomplete/bat.zsh",
                                                 "#{zsh_completions_path}/_bat")
      expect(bat).to have_received(:runCmd).with("cp", "/tmp/test/bat-assets/autocomplete/bat.bash",
