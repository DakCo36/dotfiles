require "spec_helper"
require "components/cli/ripgrep"
...
RSpec.describe Component::RipgrepComponent do
  ...
  describe "#install!" do
    it "installs ripgrep from GitHub releases" do
      ...
      allow(mock_config).to receive(:fallback_version).and_return("14.1.0")
      allow(mock_config).to receive(:owner).and_return("BurntSushi")
      allow(mock_config).to receive(:repo).and_return("ripgrep")
      ...
      allow(ripgrep).to receive(:setup_completions).and_return(["", "",
      ...
      expect(mock_github).to have_received(:download_asset).with(
        ...
        repo: "ripgrep",
        version: "latest",
        fallback_version: "14.1.0",
        asset_pattern: "ripgrep-.*-x86_64-unknown-linux-musl\\.tar\\.gz",
        fallback_asset: "ripgrep-14.1.0-x86_64-unknown-linux-musl.tar.gz",
        destination: "/tmp/test/ripgrep-assets.tar.gz"
      )
  ...
  describe "#setup_completions" do
    ...
    it "creates directories and copies completion files" do
      ...
      expect(ripgrep).to have_received(:runCmd).with("cp", "/tmp/test/ripgrep-assets/complete/_rg",
                                                     "#{zsh_completions_path}/_rg")
      expect(ripgrep).to have_received(:runCmd).with("cp", "/tmp/test/ripgrep-assets/complete/rg.bash",
