...
function install_docker_ce() {
  ...
  sudo apt-get install -y ca-certificates gnupg &>> "$INSTALL_LOG"
  ...
  sudo apt-get update &>> "$INSTALL_LOG"
  sudo apt-get install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin &>> "$INSTALL_LOG"

  sudo usermod -aG docker "$USER"
  log_info "Docker CE installed. Re-login required for docker group to take effect."
}
